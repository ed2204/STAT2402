...
Looking at the model we have fitted, we see some interesting results, interpreted in more detail later on in this report. We first want to check that the assumptions used in fitting this model are correct. In fitting the model, we have assumed that the errors are independent, $\epsilon_i$ ~ $Normal(0,\sigma^2)$, with constant variance. We can check the errors are normal by comparing them on a Normal Q-Q plot (Fig. 1.4). We are looking for the points to roughly follow the line. We can see that in this case, they do seem to follow the line, although there is a slight increase in the middle of the residuals. We can also see the residual for 'Washington D.C.' is quite far from the line. This is because the crime rate the model predicts for Washington D.C. is a lot lower than the actual crime rate. This could be due to the fact this state is the capitol of the US.




```{r echo = FALSE}
plot(crimedata.lm1, which = 2, main = "Fig 1.4 - Normal Q-Q Plot of Residuals", caption = NULL)
```
